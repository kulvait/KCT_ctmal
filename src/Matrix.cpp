    ...
    , n(n)
{
    ...
    {
        KCTERR(
            io::xprintf("The class must be initialized with proper sized list of %d elements not "
                        "%d elements.",
                        size, A_copy.size()));
    }
    A = new double[size];
...
Matrix Matrix::minorSubMatrix(uint32_t i, uint32_t j) const
{
    ...
    if(i > m - 1 || j > n - 1)
    {
        std::string errMsg = io::xprintf(
            "For minor submatrix you have to specify valid element (i, j) but not (%d, %d)!", i, j);
        LOGE << errMsg;
        throw std::runtime_error(errMsg);
    }
    Matrix out(m - 1, n - 1);
